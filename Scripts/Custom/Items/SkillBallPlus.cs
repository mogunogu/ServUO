...
namespace Server
{
	...
	{
		public static int skillsToBoost = 7;  // How many skills to boost
			...
			: base(0, 0)
		{
			...
			AddHtml(67, 41, 1153, 20, blueTen + "스킬볼" + endFont, false, false);
			AddHtml(67, 555, 1153, 20, blueTen + "레벨을 " + boostValue + "로 올릴 " + skillsToBoost + "개의 스킬을 선택해주세요." + endFont, false, false);
	...
	public class SkillBallPlus : Item
	{
		...
		public override void OnDoubleClick(Mobile m)
		{
			...
			{
				m.SendMessage("설정할 스킬 " + SkillPickGump.skillsToBoost + "개를 " + SkillPickGump.boostValue + "로 설정해주세요.");
				m.CloseGump(typeof(SkillPickGump));
				m.SendGump(new SkillPickGump(this, m));
			}
			else
				m.SendMessage("이 아이템은 가방안에 있어야 사용할수 있습니다.");
